...
<head>
    <title>RIENDSBOOK EDIT POST</title>   
          ...
          <div class="card-body">
            <h2 style=" padding-bottom: 7px; text-align: center;">Edit post</h2>
             <?php
                require 'config.php';
                $post_id= $_GET['id'];
                $query = "SELECT text from post where id =$post_id";
                $result = mysqli_query($conn, $query);
                $row = mysqli_fetch_assoc($result)
              ?>
            <form class="form-signin" action="edit_post.php?id= <?php echo $_GET['id'];?>" method="post" enctype="multipart/form-data">
              ...
              <button  class="btn btn-lg btn-primary btn-block text-uppercase" class= "btn btn-outline-light" style="background-color: black" type="submit" name="submit">Save Edit</button>
            </form>
          </div>
        </div>
      </div>
